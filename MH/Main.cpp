...
*/
pointer_t FindVisibilityBranch(HANDLE h, int pid, std::string pattern)
{
	...
	pointer_t pFound = FindPattern((pointer_t)pMemTemp, module.second, pattern);
	if (pFound)
...
bool PatchVisibilityRequirement(HANDLE h, int pid, bool patch)
{
	pointer_t pBranch;
	if (patch)
		pBranch = FindVisibilityBranch(h, pid, "83 C0 02 C2 0C 00 32 C0 5E C2 0C 00");
	else
		pBranch = FindVisibilityBranch(h, pid, "B0 01 90 C2 0C 00 B0 01 5E C2 0C 00");
	...
	*/
	...
	if (patch)
		...
		WriteProcessMemory(h, (char*)pBranch, rnShellFix, sizeof(rnShell), 0);
	return true;
}
...
int main(int argc, char ** argv)
{
	...
	while (!(GetAsyncKeyState(VK_END) & 0x8000))
	{
		...
		{
			if (PatchVisibilityRequirement(h, pid, true))
				cout << "Sig succesfully byte patched!" << endl;
			else
				cout << "Sig patch failed!" << endl;
		...
		{
			if (PatchVisibilityRequirement(h, pid, false))
				cout << "Sig succesfully restored!" << endl;
			else
				cout << "Sig restore failed!" << endl;
			Sleep(100);
		}
